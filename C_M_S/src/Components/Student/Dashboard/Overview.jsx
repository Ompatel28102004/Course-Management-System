  ...
  const handleDeadlineOperation = async (operation, deadline) => {
      try {
          ...
          if (response.status === 200 || response.status === 201) {
              console.log(`Deadline ${operation === 'add' ? 'added' : 'deleted'} successfully!`);
              window.location.reload();           
          } else {
              console.error(`Error ${operation === 'add' ? 'adding' : 'deleting'} deadline`);
          }
      } catch (error) {
          console.error(
              `Failed to connect to backend while ${operation === 'add' ? 'adding' : 'deleting'} deadline`,
              error
          );
      }
  };
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <Card className="bg-custom-blue">
            ...
            <div className="text-2xl font-bold">{data.Academic_info?.Semester}</div>
          ...
          <Card className="bg-custom-pink">
            ...
            <div className="text-2xl font-bold">{data.CourseCompleted ? Object.keys(data.CourseCompleted).length : 0}</div>
            <p className="text-xs text-gray-500">{data.Academic_info?.Total_Courses - Object.keys(data.CourseCompleted).length || 0} Remaining</p>
          </Card>
        ...
        <h2 className="text-2xl font-semibold mb-6">{data.Academic_info?.Branch}, {userId}</h2>
              ...
              <div className="relative w-40 h-40">
                <svg className="w-full h-full" viewBox="0 0 100 100">
                  ...
                  <circle
                    ...
                    strokeDashoffset={`${2 * Math.PI * 40 * (1 - (semesterProgress) / 100)}`}
                  ></circle>
                </svg>
                <div className="absolute inset-0 flex items-center justify-center">
                  <span className="text-2xl font-bold">{Math.round(semesterProgress)}%</span>
                </div>
              </div>
            </div>
            ...
            <table className="w-full">
              ...
              <tbody>
                {(data.UpcomingDeadlines && data.UpcomingDeadlines.length > 0) ? (
                  data.UpcomingDeadlines.map((evalItem, index) => (
                    <tr key={index}>
                      <td>{evalItem.heading}</td> 
          ...
          {(data.UpcomingDeadlines && data.UpcomingDeadlines.length > 0) ? (
            data.UpcomingDeadlines.map((deadline, index) => (
              <div key={index} className="flex items-center justify-between mb-4 last:mb-0">
                ...
                <X className="h-5 w-5 mr-3 text-gray-500 cursor-pointer"
